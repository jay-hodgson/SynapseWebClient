...
public class ScopeWidget implements SynapseWidgetPresenter, ScopeWidgetView.Presenter {
	...
	public void configure(EntityBundle bundle, boolean isEditable, EntityUpdatedHandler updateHandler) {
		...
		if (isVisible) {
			...
			viewScopeWidget.configure((ArrayList<String>)currentView.getScopeIds(), false, getTableType(currentView));
	...
	public void onEdit() {
		...
		editScopeWidget.configure((ArrayList<String>)currentView.getScopeIds(), true, getTableType(currentView));
		view.showModal();
	}
}
